$(function () {
    const rawData = document.getElementById("kategori-data")?.dataset.json;
    ...
    if (rawData) {
        try {
            ...
            chartData = parsed
                .filter((item) => item.nama && item.total_nominal)
                .map((item) => [item.nama, parseFloat(item.total_nominal)]);
        } catch (e) {
            console.error("Gagal parsing kategoriData:", e);
        }
    }
